              ...
              <Card style={styles.communityCard}>
                <View style={styles.cardTop}>
                  <View style={styles.iconWrap}>
                    {c.icon === 'brain' ? <Brain color={theme.action} size={22} /> : c.icon === 'gamepad' ? <Gamepad2 color={theme.action} size={22} /> : <Users color={theme.action} size={22} />}
                  </View>
                  <View style={styles.cardInfo}>
                    <Text style={styles.name}>{c.name}</Text>
                    <Text style={styles.desc}>{c.description}</Text>
                    <View style={styles.metaRow}><Users color={theme.textSecondary} size={13} /><Text style={styles.meta}>{c.members.toLocaleString('fr-FR')} membres</Text></View>
                  </View>
                </View>
                <View style={styles.footer}>
                  {isJoined ? (
                    <View style={styles.joinedBadge}><Check color={theme.success} size={16} /><Text style={styles.joinedText}>Membre</Text></View>
                  ) : (
...
const styles = StyleSheet.create({
  ...
  subtitle: { color: theme.textSecondary, fontSize: 14, fontFamily: typography.fontFamily.regular },
  list: { padding: spacing.lg, gap: spacing.md },
  communityCard: { gap: spacing.md },
  cardTop: { flexDirection: 'row', gap: spacing.md },
  iconWrap: { width: 48, height: 48, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255,106,44,0.12)', borderRadius: radii.md },
  cardInfo: { flex: 1, gap: 4 },
  name: { color: theme.textPrimary, fontSize: 17, fontWeight: '700', fontFamily: typography.fontFamily.bold },
  desc: { color: theme.textSecondary, fontSize: 13, lineHeight: 18, fontFamily: typography.fontFamily.regular },
  metaRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.xs, marginTop: 4 },
  meta: { color: theme.textSecondary, fontSize: 12, fontWeight: '500', fontFamily: typography.fontFamily.medium },

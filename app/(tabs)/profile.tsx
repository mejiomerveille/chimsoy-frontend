  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.topbar}>
        ...
        <Link href="/settings" style={styles.iconBtn}><SettingsIcon color={colors.textPrimary} size={20} /></Link>
      </View>
      ...
      <View style={styles.content}>
        <Animated.View entering={FadeInDown.duration(400)}>
          <Card tone="primary" style={styles.profileCard}>
            ...
            <View style={styles.progressSection}>
              ...
              <ProgressBar value={0.72} color={colors.accent} />
            </View>
          </Card>
        </Animated.View>
        ...
        <View style={styles.statsGrid}>
          {[
            { icon: Trophy, color: colors.accent, value: user.stats.wins, label: 'Victoires' },
            { icon: Zap, color: colors.danger, value: user.stats.losses, label: 'Défaites' },
            { icon: Flame, color: colors.success, value: `${user.stats.winRate}%`, label: 'Taux victoire' },
            { icon: Zap, color: colors.primary, value: user.stats.currentStreak, label: 'Série actuelle' },
          ].map((stat, idx) => {
          ...
          {user.badges.map((badge, idx) => (
            <Animated.View key={badge.id} entering={FadeInDown.delay(idx * 80).duration(250)} style={{ flex: 1 }}>
              <Card compact style={styles.badgeCard}>
                <View style={styles.badgeIcon}><Trophy color={colors.background} size={22} /></View>
        ...
        <View style={styles.historyList}>
          <Card compact style={styles.historyRow}>
            <View style={[styles.historyIcon, styles.historyWin]}><Trophy color={colors.success} size={16} /></View>
          ...
          <Card compact style={styles.historyRow}>
            <View style={[styles.historyIcon, styles.historyLoss]}><Flame color={colors.accent} size={16} /></View>
            <View style={styles.historyInfo}>
...
const styles = StyleSheet.create({
  ...
  iconBtn: { width: 42, height: 42, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surface, borderRadius: 21 },
  ...
  statValue: { color: colors.textPrimary, fontSize: 22, fontWeight: '700', fontFamily: typography.fontFamily.bold },
  statLabel: { color: colors.textSecondary, fontSize: 10, fontWeight: '600', fontFamily: typography.fontFamily.semiBold },
  sectionHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  sectionTitle: { color: colors.textPrimary, fontSize: 18, fontWeight: '700', fontFamily: typography.fontFamily.bold },
  sectionCount: { color: colors.textSecondary, fontSize: 12, fontWeight: '600', fontFamily: typography.fontFamily.semiBold },
  badgesList: { flexDirection: 'row', gap: spacing.md },
  badgeCard: { gap: spacing.xs },
  badgeIcon: { width: 44, height: 44, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.gold, borderRadius: 22 },
  badgeLabel: { color: colors.textPrimary, fontSize: 14, fontWeight: '700', fontFamily: typography.fontFamily.bold },
  badgeDesc: { color: colors.textSecondary, fontSize: 12, lineHeight: 16, fontFamily: typography.fontFamily.regular },
  ...
  historyInfo: { flex: 1 },
  historyLabel: { color: colors.textPrimary, fontSize: 14, fontWeight: '700', fontFamily: typography.fontFamily.bold },
  historyOpp: { color: colors.textSecondary, fontSize: 12, marginTop: 2, fontFamily: typography.fontFamily.regular },
  historyPoints: { color: colors.success, fontSize: 16, fontWeight: '700', fontFamily: typography.fontFamily.bold },
  historyPointsLoss: { color: colors.textSecondary, fontSize: 16, fontWeight: '700', fontFamily: typography.fontFamily.bold },
});

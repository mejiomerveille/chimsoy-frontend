              ...
              <Card style={styles.tournamentCard}>
                <View style={styles.cardTop}>
                  <View style={styles.iconWrap}>
                    {t.gameId === 'quiz' ? <Brain color={theme.action} size={22} /> : <Gamepad2 color={theme.action} size={22} />}
                  </View>
                  {t.status === 'live' ? (
                ...
                <View style={styles.stats}>
                  <View style={styles.stat}><Users color={theme.textSecondary} size={15} /><Text style={styles.statText}>{t.participants}/{t.maxParticipants}</Text></View>
                ...
                <View style={styles.footer}>
                  ...
                  {isJoined ? (
                    <View style={styles.joinedBadge}><Check color={theme.success} size={16} /><Text style={styles.joinedText}>Inscrit</Text></View>
                  ) : (
                    <Button
                      label={isFull ? 'Complet' : "S'inscrire"}
                      disabled={isFull}
                      icon={!isFull && <Zap color={theme.background} size={16} />}
...
const styles = StyleSheet.create({
  ...
  filterTab: { paddingHorizontal: spacing.md, paddingVertical: 8, borderRadius: 999, backgroundColor: theme.surface },
  filterTabActive: { backgroundColor: theme.action },
  filterText: { color: theme.textSecondary, fontSize: 13, fontWeight: '700', fontFamily: typography.fontFamily.bold },
  filterTextActive: { color: theme.background },
  list: { padding: spacing.lg, gap: spacing.md },
  tournamentCard: { gap: spacing.sm },
  cardTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  iconWrap: { width: 44, height: 44, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255,106,44,0.12)', borderRadius: radii.sm },
  ...
  liveDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: theme.action },
  liveBadgeText: { color: theme.action, fontSize: 10, fontWeight: '700', fontFamily: typography.fontFamily.bold, letterSpacing: 0.8 },
  name: { color: theme.textPrimary, fontSize: 18, fontWeight: '700', fontFamily: typography.fontFamily.bold },
  stats: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  stat: { flexDirection: 'row', alignItems: 'center', gap: spacing.xs },
  statText: { color: theme.textSecondary, fontSize: 13, fontWeight: '500', fontFamily: typography.fontFamily.medium },
  prize: { color: theme.reward, fontSize: 13, fontWeight: '700', fontFamily: typography.fontFamily.bold },
  progressBar: { marginVertical: 4 },
  progressTrack: { height: 6, backgroundColor: theme.background, borderRadius: 3, overflow: 'hidden' },
  progressFill: { height: '100%', backgroundColor: theme.action, borderRadius: 3 },
  footer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: spacing.xs },
  fee: { color: theme.textPrimary, fontSize: 14, fontWeight: '700', fontFamily: typography.fontFamily.bold },
  ...
  joinedText: { color: theme.success, fontSize: 14, fontWeight: '700', fontFamily: typography.fontFamily.bold },
});
